...
void Comms_DummyFinished(void* arriveData)
{
	...
	justDummied = realloc((2 * sizeof(uint32_t)) + pathLength);			//Importante reallocar!!
...
void Comms_AbrirFinished(void* arriveData)
{
	...
	dictionary_putMAESTRO(nextToUnlock->openedFilesUpdate, (char*)(params->parts[1]), (uint32_t*)(params->parts[2]));
...
void Comms_DummyAtDAM(void* arrivedData)
{
